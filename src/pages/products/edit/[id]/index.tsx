...
import { useRoqClient, useProductFindFirst } from 'lib/roq';
...
function ProductEditPage() {
  ...
  const { data, error, isLoading, mutate } = useProductFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ProductInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.product.update({
        data: values as RoqTypes.product,
        where: {
          id,
        },
      });
